...
module SeparateChainBag =
        ...
        let updatedChains =
            ...
            |> List.collect (fun chain ->
                if chain.Hash = h then
                    ...
                    if filteredElements.IsEmpty then [] else [{ chain with Elements = filteredElements }]
                else
                    [chain])
        { Chains = updatedChains }
    ...
    let compareBags (bag1: Bag<'T>) (bag2: Bag<'T>) : bool =
        let compareChains (chain1: Chain<'T>) (chain2: Chain<'T>) =
            chain1.Hash = chain2.Hash && List.sort chain1.Elements = List.sort chain2.Elements
        ...
        if List.length bag1.Chains = List.length bag2.Chains then
            ...
            sortedChains1
            |> List.forall2 compareChains sortedChains2
        else
            false
    ...
    let merge (bag1: Bag<'T>) (bag2: Bag<'T>) : Bag<'T> =
        ...
        let mergedBag =
            bag2.Chains
            |> List.fold (fun acc chain -> chain.Elements |> List.fold (fun b e -> { b with Chains = addItemToBag b e }) acc) bag1

        let filteredBag = { mergedBag with Chains = List.filter (fun chain -> not (List.isEmpty chain.Elements)) mergedBag.Chains }
        if List.isEmpty filteredBag.Chains then { Chains = [] } else filteredBag
